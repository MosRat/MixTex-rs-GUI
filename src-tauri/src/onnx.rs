...
//
// impl MixTexOnnx {
//     fn prefill( &mut self, img: &[f32]) -> Result<(usize, Array<f32, IxDyn>, SessionOutputs)> {
...
//     fn decode_once<'a>(
//         &mut self,
//         state: (usize, Array<f32, IxDyn>, SessionOutputs<'a>),
//     ) -> Result<(usize, Array<f32, IxDyn>, SessionOutputs<'a>)> {
...
//         })
//     }
//     fn inference(&mut self, img: &[f32]) -> Result<String> {
...
//     }
//     fn generate<F>(&mut self, img: &[f32], mut callback: F) -> Result<String>
//     where
//         F: FnMut(String) -> bool,
//     {
//         let check_rate = MAX_LENGTH / 64;
...
impl MixTexOnnx {
    fn prefill(&mut self, _img: &[f32]) -> Result<(usize, (), ())> {
        Ok((0, (), ()))
    }

    fn decode_once<'a>(&mut self, _state: (usize, (), ())) -> Result<(usize, (), ())> {
        Ok((0, (), ()))
    }
}
...
impl OcrModel for MixTexOnnx {
    ...
    fn inference(& self, _img: &[f32]) -> Result<String> {
        info!("Time cost: 0ms");
        Ok(String::new())
    }

    fn generate<F>(& self, _img: &[f32], _callback: F) -> Result<String>
    where
        F: FnMut(String) -> bool,
    {
        Ok(String::new())
